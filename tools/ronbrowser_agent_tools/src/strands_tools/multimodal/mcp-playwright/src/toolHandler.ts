...
import { BROWSER_TOOLS, API_TOOLS, ELECTRON_PAGE_TOOLS } from './tools.js';
...
import { ensureElectronBrowser, resetElectronBrowserState } from './electron/connection.js';
import { 
  startCodegenSession,
  endCodegenSession,
  getCodegenSession,
...
import {
  ...
  ElectronApiGetTool,
  ElectronApiSetTool,
  ElectronApiSearchTool,
  ElectronApiDescribeTool,
  ElectronHandleCallTool,
  ElectronHandleGetTool,
  ElectronHandleSetTool,
  ElectronHandleReleaseTool,
  ElectronHandleListTool,
  ElectronEvalMainTool
} from './tools/electron/api.js';
import { ElectronCdpSendTool, ElectronCdpDomainsTool, ElectronCdpSearchTool } from './tools/electron/cdp.js';
import {
  ElectronDevToolsOpenTool,
  ElectronDevToolsCloseTool,
  ElectronDevToolsToggleTool,
  ElectronDevToolsInspectTool
} from './tools/electron/devtools.js';
import { ElectronEmbedBrowserTool } from './tools/electron/embed.js';
import { ElectronStatusTool } from './tools/electron/status.js';

// Global state
let browser: Browser | undefined;
let page: Page | undefined;
let currentBrowserType: 'chromium' | 'firefox' | 'webkit' = 'chromium';
let browserConnection: 'launch' | 'cdp' = 'launch';
...
export function resetBrowserState() {
  ...
  browserConnection = 'launch';
  resetElectronBrowserState();
}
/**
...
let clickAtCoordinatesTool: ClickAtCoordinatesTool;
let typeAtCoordinatesTool: TypeAtCoordinatesTool;
...
let electronStatusTool: ElectronStatusTool;
let electronApiCallTool: ElectronApiCallTool;
let electronApiGetTool: ElectronApiGetTool;
let electronApiSetTool: ElectronApiSetTool;
let electronApiSearchTool: ElectronApiSearchTool;
let electronApiDescribeTool: ElectronApiDescribeTool;
let electronHandleCallTool: ElectronHandleCallTool;
let electronHandleGetTool: ElectronHandleGetTool;
let electronHandleSetTool: ElectronHandleSetTool;
let electronHandleReleaseTool: ElectronHandleReleaseTool;
let electronHandleListTool: ElectronHandleListTool;
let electronEvalMainTool: ElectronEvalMainTool;
let electronCdpSendTool: ElectronCdpSendTool;
let electronCdpDomainsTool: ElectronCdpDomainsTool;
let electronCdpSearchTool: ElectronCdpSearchTool;
let electronDevToolsOpenTool: ElectronDevToolsOpenTool;
let electronDevToolsCloseTool: ElectronDevToolsCloseTool;
let electronDevToolsToggleTool: ElectronDevToolsToggleTool;
let electronDevToolsInspectTool: ElectronDevToolsInspectTool;
let electronEmbedBrowserTool: ElectronEmbedBrowserTool;
...
export async function ensureBrowser(browserSettings?: BrowserSettings) {
  ...
  try {
    const useElectronTarget = electronConfig.mode === 'electron'
      || (electronConfig.mode === 'headless' && electronConfig.headlessTarget === 'electron');
    if (useElectronTarget) {
      const { browser: electronBrowser, page: electronPage } = await ensureElectronBrowser();
      const pageChanged = page !== electronPage;
    ...
    if (!browser) {
      const { viewport, userAgent, headless: requestedHeadless = false, browserType = 'chromium' } = browserSettings ?? {};
      const headless = electronConfig.mode === 'headless' ? true : requestedHeadless;
    ...
    if (useElectronTarget) {
      // Never fall back to launching a standalone browser in Electron mode.
      throw error;
    }
...
function initializeTools(server: any) {
  ...
  if (!clickAtCoordinatesTool) clickAtCoordinatesTool = new ClickAtCoordinatesTool(server);
  if (!typeAtCoordinatesTool) typeAtCoordinatesTool = new TypeAtCoordinatesTool(server);
  ...
  // Electron tools
  if (!electronStatusTool) electronStatusTool = new ElectronStatusTool();
  if (!electronApiCallTool) electronApiCallTool = new ElectronApiCallTool();
  if (!electronApiGetTool) electronApiGetTool = new ElectronApiGetTool();
  if (!electronApiSetTool) electronApiSetTool = new ElectronApiSetTool();
  if (!electronApiSearchTool) electronApiSearchTool = new ElectronApiSearchTool();
  if (!electronApiDescribeTool) electronApiDescribeTool = new ElectronApiDescribeTool();
  if (!electronHandleCallTool) electronHandleCallTool = new ElectronHandleCallTool();
  if (!electronHandleGetTool) electronHandleGetTool = new ElectronHandleGetTool();
  if (!electronHandleSetTool) electronHandleSetTool = new ElectronHandleSetTool();
  if (!electronHandleReleaseTool) electronHandleReleaseTool = new ElectronHandleReleaseTool();
  if (!electronHandleListTool) electronHandleListTool = new ElectronHandleListTool();
  if (!electronEvalMainTool) electronEvalMainTool = new ElectronEvalMainTool();
  if (!electronCdpSendTool) electronCdpSendTool = new ElectronCdpSendTool();
  if (!electronCdpDomainsTool) electronCdpDomainsTool = new ElectronCdpDomainsTool();
  if (!electronCdpSearchTool) electronCdpSearchTool = new ElectronCdpSearchTool();
  if (!electronDevToolsOpenTool) electronDevToolsOpenTool = new ElectronDevToolsOpenTool();
  if (!electronDevToolsCloseTool) electronDevToolsCloseTool = new ElectronDevToolsCloseTool();
  if (!electronDevToolsToggleTool) electronDevToolsToggleTool = new ElectronDevToolsToggleTool();
  if (!electronDevToolsInspectTool) electronDevToolsInspectTool = new ElectronDevToolsInspectTool();
  if (!electronEmbedBrowserTool) electronEmbedBrowserTool = new ElectronEmbedBrowserTool();
}
...
export async function handleToolCall(
  ...
  try {
    ...
    if (browser && !browser.isConnected() && (BROWSER_TOOLS.includes(name) || ELECTRON_PAGE_TOOLS.includes(name))) {
      try {
  ...
  if (BROWSER_TOOLS.includes(name) || ELECTRON_PAGE_TOOLS.includes(name)) {
    ...
    switch (name) {
      ...
      case "playwright_click_coordinates":
        return await clickAtCoordinatesTool.execute(args, context);
      
      case "playwright_type_coordinates":
        return await typeAtCoordinatesTool.execute(args, context);
      ...
      case "electron_status":
        return await electronStatusTool.execute(args, context);
      case "electron_api_call":
        return await electronApiCallTool.execute(args, context);
      case "electron_api_get":
        return await electronApiGetTool.execute(args, context);
      case "electron_api_set":
        return await electronApiSetTool.execute(args, context);
      case "electron_api_search":
        return await electronApiSearchTool.execute(args, context);
      case "electron_api_describe":
        return await electronApiDescribeTool.execute(args, context);
      case "electron_handle_call":
        return await electronHandleCallTool.execute(args, context);
      case "electron_handle_get":
        return await electronHandleGetTool.execute(args, context);
      case "electron_handle_set":
        return await electronHandleSetTool.execute(args, context);
      case "electron_handle_release":
        return await electronHandleReleaseTool.execute(args, context);
      case "electron_handle_list":
        return await electronHandleListTool.execute(args, context);
      case "electron_eval_main":
        return await electronEvalMainTool.execute(args, context);
      case "electron_cdp_send":
        return await electronCdpSendTool.execute(args, context);
      case "electron_cdp_domains":
        return await electronCdpDomainsTool.execute(args, context);
      case "electron_cdp_search":
        return await electronCdpSearchTool.execute(args, context);
      case "electron_devtools_open":
        return await electronDevToolsOpenTool.execute(args, context);
      case "electron_devtools_close":
        return await electronDevToolsCloseTool.execute(args, context);
      case "electron_devtools_toggle":
        return await electronDevToolsToggleTool.execute(args, context);
      case "electron_devtools_inspect":
        return await electronDevToolsInspectTool.execute(args, context);
      case "electron_embed_browser":
        return await electronEmbedBrowserTool.execute(args, context);
 ...
 * Get screenshots
 */
export function getScreenshots(): Map<string, any> {
