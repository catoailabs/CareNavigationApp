...
def bright_data(
    ...
    try:
        ...
        if action == "scrape_as_markdown":
            ...
            result_text = client.scrape_as_markdown(url, zone)
            ...
            return {"status": "success", "content": [{"text": f"Screenshot saved to {output_path_result}"}]}

        elif action == "search_engine":
            if not query:
                raise ValueError("query is required for search_engine action")
            result_text = client.search_engine(
                query=query,
                engine=engine,
                zone=zone,
                language=language,
            ...
            # Try to build structured search results for __ui_data__
            ui_results = []
            if return_json:
                try:
                    parsed = json.loads(result_text) if isinstance(result_text, str) else result_text
                    # Google SERP JSON typically has organic results
                    organic = parsed.get("organic", []) if isinstance(parsed, dict) else []
                    for i, r in enumerate(organic):
                        ui_results.append({
                            "id": f"bright-data-{i}-{hash(r.get('link', r.get('url', ''))) % 100000}",
                            "title": r.get("title", ""),
                            "url": r.get("link", r.get("url", "")),
                            "snippet": r.get("description", r.get("snippet", "")),
                            "source": engine or "google",
                        })
