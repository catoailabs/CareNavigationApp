...
async def tavily_search(
    ...
    try:
        ...
        for i, r in enumerate(data.get("results", [])):
            ui_results.append({
                "id": f"tavily-{i}-{hash(r.get('url', '')) % 100000}",
                "title": r.get("title", ""),
                "url": r.get("url", ""),
...
async def tavily_extract(
    ...
    try:
        ...
        # Build normalized source results for __ui_data__
        ui_results = []
        for i, r in enumerate(data.get("results", [])):
            raw = r.get("raw_content", "")
            snippet = (raw[:200].strip() + "...") if raw and len(raw) > 200 else (raw.strip() if raw else "")
            ui_results.append({
                "id": f"tavily-extract-{i}-{hash(r.get('url', '')) % 100000}",
                "title": r.get("url", ""),
                "url": r.get("url", ""),
                "snippet": snippet,
                "favicon": r.get("favicon"),
        ...
        # Determine the query string (use the first URL or a comma-separated list)
        extract_query = urls[0] if isinstance(urls, list) and urls else (urls if isinstance(urls, str) else "")
...
async def tavily_crawl(
    ...
    try:
        ...
        for i, r in enumerate(data.get("results", [])):
            ...
            snippet = (raw[:200].strip() + "...") if raw and len(raw) > 200 else (raw.strip() if raw else "")
            ui_results.append({
                "id": f"tavily-crawl-{i}-{hash(r.get('url', '')) % 100000}",
                "title": r.get("url", ""),
                "url": r.get("url", ""),
                "snippet": snippet,
                "favicon": r.get("favicon"),
