...
def _container_exec(cmd_args, **kwargs):
    return subprocess.run(["docker", "exec", "ron-agent-desktop"] + cmd_args, **kwargs)


def _container_exec_i(cmd_args, **kwargs):
    return subprocess.run(["docker", "exec", "-i", "ron-agent-desktop"] + cmd_args, **kwargs)
...
class ContainerFile:
    def __init__(self, path, mode):
        ...
        self.content = ""
        if "r" in mode:
            res = _container_exec(["cat", path], capture_output=True, text=True)
            if res.returncode != 0:
                raise FileNotFoundError(f"No such file: {path}")
            self.content = res.stdout
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if "w" in self.mode:
            _container_exec_i(["sh", "-c", f"cat > '{self.path}'"], input=self.content, capture_output=True, text=True)
...
def _mock_open(path, mode="r", **kwargs):
    if str(path).startswith("/workspace"):
        return ContainerFile(path, mode)
...
os.path.isfile = lambda p: (
    _container_exec(["test", "-f", p]).returncode == 0 if str(p).startswith("/workspace") else _orig_isfile(p)
)

_orig_isdir = os.path.isdir
os.path.isdir = lambda p: (
    _container_exec(["test", "-d", p]).returncode == 0 if str(p).startswith("/workspace") else _orig_isdir(p)
)

_orig_exists = os.path.exists
os.path.exists = lambda p: (
    _container_exec(["test", "-e", p]).returncode == 0 if str(p).startswith("/workspace") else _orig_exists(p)
)
...
def _mock_makedirs(name, mode=511, exist_ok=False):
    if str(name).startswith("/workspace"):
        _container_exec(["mkdir", "-p", name])
    else:
...
def _mock_listdir(path="."):
    if str(path).startswith("/workspace"):
        res = _container_exec(["ls", "-1A", path], capture_output=True, text=True)
        if res.returncode != 0:
            raise FileNotFoundError(f"No such dir: {path}")
        return [x for x in res.stdout.split("\n") if x]
...
def _mock_remove(path, *, dir_fd=None):
    if str(path).startswith("/workspace"):
        _container_exec(["rm", "-f", path])
    else:
        _orig_remove(path, dir_fd=dir_fd)
...
def _mock_copy2(src, dst, *, follow_symlinks=True):
    if str(src).startswith("/workspace") or str(dst).startswith("/workspace"):
        _container_exec(["cp", "-p", src, dst])
    else:
        _orig_copy2(src, dst, follow_symlinks=follow_symlinks)
...
def editor(
    ...
    try:
        ...
        # Build __ui_data__ for frontend rendering
        ui_data = _build_ui_data(command, path, file_text, new_str, old_str)
...
def _build_ui_data(
    ...
    """Build __ui_data__ payload for the frontend IDE/CodeBlock components."""
    ...
    if command == "view":
        # Show the file content that was viewed
        try:
            content = get_last_content(path)
            if content is None:
                with open(path, "r") as f:
    ...
    elif command in ("str_replace", "pattern_replace", "insert"):
        # Show the new content after modification
        content = get_last_content(path)
        if content is None:
            try:
                with open(path, "r") as f:
